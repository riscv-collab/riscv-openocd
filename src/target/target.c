...
/**
 * @fn uint32_t psram_init(struct target *target,uint8_t qspinum, int flash_size,uint8_t fthresh)
 * 
 * @brief Used to configure QSPI for PSRAM initialisation.
 * 
 * @param qspinum The parameter \a qspinum is an unsigned integer that represents the QSPI instance number.
 * @param ram_size The parameter \a ram_size specifies psram size.
 * @param fthresh The parameter \a fthresh specifies if 0 - 8 bit mode,1-16 mode,3 - 32 bit mode,7- 64 bit mode.             .
 * 
 * 
 * @return SUCCESS if operation is successful and ENODEV if invalid instance number.
 */
