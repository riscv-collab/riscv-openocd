	...
	while (next_address < end_address) {
		...
		for (uint32_t i = (next_address - address) / size; i < count - 1; i++) {
			...
			for (size_t k = 0; k <= last_key; ++k) {
				...
				/* This can be written as: n_bits = MIN(32, 8 * size), but I think
				 * we'd better to be explicit on how n_bits is calculated */
				const size_t n_bits = (k != last_key) ? 32 : (8 * size - k * 4 * 8);
				assert(n_bits <= 32);
				buf_set_u32(buffer + i * size + k * 4, 0, n_bits, sbvalue[k]);
			}
			riscv_batch_free(batch);
