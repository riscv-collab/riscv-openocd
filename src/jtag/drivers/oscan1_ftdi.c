 ...
 *
 * This driver is based on ftdi.c, but is a variant that assumes a two-wire
 * interface to the signals associated with the OSCAN1 protocol of cJTAG.
 * The signaling protocol is different than for 4-wire JTAG, but it is possible
 * to idiomatically drive the MPSSE interface to achieve the necessary
 * signaling, if the debug adapter has a way to drive a pin bidirectionally.
 * This driver assumes that TMS=1/TDI=N will drive an output on TMSC,
 * and TMS=0 will let the target drive TMSC.  OSCAN1 transactions involve
 * 3 TCKs: one to transmit an nTDI bit value, one to transmit a TMS bit value,
 * and one to receive a bit of TDO output.
 */
...
static char *oscan1_ftdi_device_desc;
static char *oscan1_ftdi_serial;
static char *oscan1_ftdi_location;
static uint8_t oscan1_ftdi_channel;
static uint8_t oscan1_ftdi_jtag_mode = JTAG_MODE;

static bool swd_mode;
static bool oscan1_mode = true;

#define MAX_USB_IDS 8
/* vid = pid = 0 marks the end of the list */
static uint16_t oscan1_ftdi_vid[MAX_USB_IDS + 1] = { 0 };
static uint16_t oscan1_ftdi_pid[MAX_USB_IDS + 1] = { 0 };
...
static int oscan1_ftdi_swd_switch_seq(enum swd_special_seq seq);
...
static int oscan1_ftdi_set_signal(const struct signal *s, char value)
{
	bool data;
	bool oe;
...
static int oscan1_ftdi_get_signal(const struct signal *s, uint16_t * value_out)
{
	uint8_t data_low = 0;
	uint8_t data_high = 0;
...
static void move_to_state(tap_state_t goal_state)
{
	...
	*/
	...
	oscan1_mpsse_clock_tms_cs_out(mpsse_ctx, &tms_bits, 0, tms_count, false, oscan1_ftdi_jtag_mode);
#if 0
	mpsse_clock_tms_cs_out(mpsse_ctx,
		...
		tms_count,
		false,
		oscan1_ftdi_jtag_mode);
#endif	
}

static int oscan1_ftdi_speed(int speed)
{
	int retval;
	...
	if (!swd_mode && speed >= 10000000 && oscan1_ftdi_jtag_mode != JTAG_MODE_ALT)
		LOG_INFO("ftdi: if you experience problems at higher adapter clocks, try "
			 "the command \"oscan1_ftdi_tdo_sample_edge falling\"");
	return ERROR_OK;
}

static int oscan1_ftdi_speed_div(int speed, int *khz)
{
	*khz = speed / 1000;
	return ERROR_OK;
}

static int oscan1_ftdi_khz(int khz, int *jtag_speed)
{
...
static void oscan1_ftdi_end_state(tap_state_t state)
{
...
static void oscan1_ftdi_execute_runtest(struct jtag_command *cmd)
{
	int i;
	uint8_t zero = 0;
	...
	/* TODO: Reuse oscan1_ftdi_execute_stableclocks */
	i = cmd->cmd.runtest->num_cycles;
	while (i > 0) {
		/* there are no state transitions in this code, so omit state tracking */
		unsigned this_len = i > 7 ? 7 : i;
		oscan1_mpsse_clock_tms_cs_out(mpsse_ctx, &zero, 0, this_len, false, oscan1_ftdi_jtag_mode);
		i -= this_len;
	}

	oscan1_ftdi_end_state(cmd->cmd.runtest->end_state);
...
static void oscan1_ftdi_execute_statemove(struct jtag_command *cmd)
{
	DEBUG_JTAG_IO("statemove end in %s",
		tap_state_name(cmd->cmd.statemove->end_state));

	oscan1_ftdi_end_state(cmd->cmd.statemove->end_state);
...
static void oscan1_ftdi_execute_pathmove(struct jtag_command *cmd)
{
...
static void oscan1_ftdi_execute_scan(struct jtag_command *cmd)
{
	...
	oscan1_ftdi_end_state(cmd->cmd.scan->end_state);
	...
	for (int i = 0; i < cmd->cmd.scan->num_fields; i++, field++) {
		...
		if (i == cmd->cmd.scan->num_fields - 1 && tap_get_state() != tap_get_end_state()) {
			...
			oscan1_mpsse_clock_data(mpsse_ctx,
				field->out_value,
				0,
				field->in_value,
				0,
			...
			oscan1_mpsse_clock_tms_cs(mpsse_ctx,
					&tms_bits,
					0,
					field->in_value,
					field->num_bits - 1,
...
static void oscan1_ftdi_execute_reset(struct jtag_command *cmd)
{
	...
	if (cmd->cmd.reset->srst == 1) {
		...
		if (jtag_get_reset_config() & RESET_SRST_PUSH_PULL)
			oscan1_ftdi_set_signal(srst, '1');
		else
			oscan1_ftdi_set_signal(srst, 'z');
	}
...
static void oscan1_ftdi_execute_sleep(struct jtag_command *cmd)
{
...
static void oscan1_ftdi_execute_stableclocks(struct jtag_command *cmd)
{
	...
	while (num_cycles > 0) {
		...
		oscan1_mpsse_clock_tms_cs_out(mpsse_ctx, &tms, 0, this_len, false, oscan1_ftdi_jtag_mode);
		num_cycles -= this_len;
	}
...
static void oscan1_ftdi_execute_command(struct jtag_command *cmd)
{
	switch (cmd->type) {
		case JTAG_RESET:
		...
		case JTAG_PATHMOVE:
			...
			break;
		case JTAG_SCAN:
			oscan1_ftdi_execute_scan(cmd);
			break;
		case JTAG_SLEEP:
			oscan1_ftdi_execute_sleep(cmd);
			break;
		case JTAG_STABLECLOCKS:
			oscan1_ftdi_execute_stableclocks(cmd);
			break;
		case JTAG_TMS:
			oscan1_ftdi_execute_tms(cmd);
			break;
		default:
...
static int oscan1_ftdi_execute_queue(void)
{
	/* blink, if the current layout has that feature */
	struct signal *led = find_signal_by_name("LED");
	if (led)
		oscan1_ftdi_set_signal(led, '1');

	for (struct jtag_command *cmd = jtag_command_queue; cmd; cmd = cmd->next) {
		/* fill the write buffer with the desired command */
		oscan1_ftdi_execute_command(cmd);
	}

	if (led)
		oscan1_ftdi_set_signal(led, '0');
...
static int oscan1_ftdi_initialize(void)
{
	...
	for (int i = 0; oscan1_ftdi_vid[i] || oscan1_ftdi_pid[i]; i++) {
		mpsse_ctx = mpsse_open(&oscan1_ftdi_vid[i], &oscan1_ftdi_pid[i], oscan1_ftdi_device_desc,
				oscan1_ftdi_serial, oscan1_ftdi_location, oscan1_ftdi_channel);
		if (mpsse_ctx)
			break;
	}
...
static int oscan1_ftdi_quit(void)
{
	...
	free(oscan1_ftdi_device_desc);
	free(oscan1_ftdi_serial);
	free(oscan1_ftdi_location);
...
COMMAND_HANDLER(oscan1_ftdi_handle_device_desc_command)
{
	if (CMD_ARGC == 1) {
		if (oscan1_ftdi_device_desc)
			free(oscan1_ftdi_device_desc);
		oscan1_ftdi_device_desc = strdup(CMD_ARGV[0]);
	} else {
		LOG_ERROR("expected exactly one argument to oscan1_ftdi_device_desc <description>");
	}

	return ERROR_OK;
}

COMMAND_HANDLER(oscan1_ftdi_handle_serial_command)
{
	if (CMD_ARGC == 1) {
		if (oscan1_ftdi_serial)
			free(oscan1_ftdi_serial);
		oscan1_ftdi_serial = strdup(CMD_ARGV[0]);
	} else {
...
COMMAND_HANDLER(oscan1_ftdi_handle_location_command)
{
	if (CMD_ARGC == 1) {
		if (oscan1_ftdi_location)
			free(oscan1_ftdi_location);
		oscan1_ftdi_location = strdup(CMD_ARGV[0]);
	} else {
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
COMMAND_HANDLER(oscan1_ftdi_handle_channel_command)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(u8, CMD_ARGV[0], oscan1_ftdi_channel);
	else
		return ERROR_COMMAND_SYNTAX_ERROR;
...
COMMAND_HANDLER(oscan1_ftdi_handle_layout_init_command)
{
	if (CMD_ARGC != 2)
		return ERROR_COMMAND_SYNTAX_ERROR;
...
COMMAND_HANDLER(oscan1_ftdi_handle_layout_signal_command)
{
	if (CMD_ARGC < 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
...
COMMAND_HANDLER(oscan1_ftdi_handle_set_signal_command)
{
	if (CMD_ARGC < 2)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	case 'Z':
		...
		if (CMD_ARGV[1][1] == '\0') {
			oscan1_ftdi_set_signal(sig, *CMD_ARGV[1]);
			break;
		}
		/* fallthrough */
	default:
...
COMMAND_HANDLER(oscan1_ftdi_handle_get_signal_command)
{
	if (CMD_ARGC < 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	int ret = oscan1_ftdi_get_signal(sig, &sig_data);
	if (ret != ERROR_OK)
		return ret;
...
COMMAND_HANDLER(oscan1_ftdi_handle_vid_pid_command)
{
	if (CMD_ARGC > MAX_USB_IDS * 2) {
		LOG_WARNING("ignoring extra IDs in oscan1_ftdi_vid_pid "
			"(maximum is %d pairs)", MAX_USB_IDS);
		CMD_ARGC = MAX_USB_IDS * 2;
	}
	if (CMD_ARGC < 2 || (CMD_ARGC & 1)) {
		LOG_WARNING("incomplete oscan1_ftdi_vid_pid configuration directive");
		if (CMD_ARGC < 2)
			return ERROR_COMMAND_SYNTAX_ERROR;
	...
	for (i = 0; i < CMD_ARGC; i += 2) {
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[i], oscan1_ftdi_vid[i >> 1]);
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[i + 1], oscan1_ftdi_pid[i >> 1]);
	}
	 ...
	 * oscan1_ftdi_vid_pid.
	 */
	oscan1_ftdi_vid[i >> 1] = oscan1_ftdi_pid[i >> 1] = 0;

	return ERROR_OK;
}

COMMAND_HANDLER(oscan1_ftdi_handle_tdo_sample_edge_command)
{
	Jim_Nvp *n;
	static const Jim_Nvp nvp_oscan1_ftdi_jtag_modes[] = {
	...
	if (CMD_ARGC > 0) {
		n = Jim_Nvp_name2value_simple(nvp_oscan1_ftdi_jtag_modes, CMD_ARGV[0]);
		if (n->name == NULL)
			return ERROR_COMMAND_SYNTAX_ERROR;
		oscan1_ftdi_jtag_mode = n->value;

	}

	n = Jim_Nvp_value2name_simple(nvp_oscan1_ftdi_jtag_modes, oscan1_ftdi_jtag_mode);
	command_print(CMD_CTX, "ftdi samples TDO on %s edge of TCK", n->name);

	return ERROR_OK;
}

static const struct command_registration oscan1_ftdi_command_handlers[] = {
	{
		.name = "oscan1_ftdi_device_desc",
		.handler = &oscan1_ftdi_handle_device_desc_command,
		.mode = COMMAND_CONFIG,
		.help = "set the USB device description of the FTDI device",
		.usage = "description_string",
	},
	{
		.name = "oscan1_ftdi_serial",
		.handler = &oscan1_ftdi_handle_serial_command,
		.mode = COMMAND_CONFIG,
...
#ifdef HAVE_LIBUSB_GET_PORT_NUMBERS
	{
		.name = "oscan1_ftdi_location",
		.handler = &oscan1_ftdi_handle_location_command,
		.mode = COMMAND_CONFIG,
...
#endif
	{
		.name = "oscan1_ftdi_channel",
		.handler = &oscan1_ftdi_handle_channel_command,
		.mode = COMMAND_CONFIG,
		.help = "set the channel of the FTDI device that is used as JTAG",
		.usage = "(0-3)",
	},
	{
		.name = "oscan1_ftdi_layout_init",
		.handler = &oscan1_ftdi_handle_layout_init_command,
		.mode = COMMAND_CONFIG,
	...
	{
		.name = "oscan1_ftdi_layout_signal",
		.handler = &oscan1_ftdi_handle_layout_signal_command,
		.mode = COMMAND_ANY,
	...
	{
		.name = "oscan1_ftdi_set_signal",
		.handler = &oscan1_ftdi_handle_set_signal_command,
		.mode = COMMAND_EXEC,
		.help = "control a layout-specific signal",
		.usage = "name (1|0|z)",
	},
	{
		.name = "oscan1_ftdi_get_signal",
		.handler = &oscan1_ftdi_handle_get_signal_command,
		.mode = COMMAND_EXEC,
		.help = "read the value of a layout-specific signal",
		.usage = "name",
	},
	{
		.name = "oscan1_ftdi_vid_pid",
		.handler = &oscan1_ftdi_handle_vid_pid_command,
		.mode = COMMAND_CONFIG,
		.help = "the vendor ID and product ID of the FTDI device",
		.usage = "(vid pid)* ",
	},
	{
		.name = "oscan1_ftdi_tdo_sample_edge",
		.handler = &oscan1_ftdi_handle_tdo_sample_edge_command,
		.mode = COMMAND_ANY,
...
static int oscan1_ftdi_swd_init(void)
{
...
static void oscan1_ftdi_swd_swdio_en(bool enable)
{
	struct signal *oe = find_signal_by_name("SWDIO_OE");
	if (oe) {
		if (oe->data_mask)
			oscan1_ftdi_set_signal(oe, enable ? '1' : '0');
		else {
 ...
 * @return
 */
static int oscan1_ftdi_swd_run_queue(void)
{
...
static void oscan1_ftdi_swd_queue_cmd(uint8_t cmd, uint32_t *dst, uint32_t data, uint32_t ap_delay_clk)
{
	if (swd_cmd_queue_length >= swd_cmd_queue_alloced) {
		...
		queued_retval = oscan1_ftdi_swd_run_queue();
...
static void oscan1_ftdi_swd_read_reg(uint8_t cmd, uint32_t *value, uint32_t ap_delay_clk)
{
	assert(cmd & SWD_CMD_RnW);
	oscan1_ftdi_swd_queue_cmd(cmd, value, 0, ap_delay_clk);
}

static void oscan1_ftdi_swd_write_reg(uint8_t cmd, uint32_t value, uint32_t ap_delay_clk)
{
	assert(!(cmd & SWD_CMD_RnW));
	oscan1_ftdi_swd_queue_cmd(cmd, NULL, value, ap_delay_clk);
}

static int_least32_t oscan1_ftdi_swd_frequency(int_least32_t hz)
{
	if (hz > 0)
...
static const struct swd_driver oscan1_ftdi_swd = {
	.init = oscan1_ftdi_swd_init,
	.frequency = oscan1_ftdi_swd_frequency,
	.switch_seq = oscan1_ftdi_swd_switch_seq,
	.read_reg = oscan1_ftdi_swd_read_reg,
	.write_reg = oscan1_ftdi_swd_write_reg,
	.run = oscan1_ftdi_swd_run_queue,
};

static const char * const oscan1_ftdi_transports[] = { "jtag", "swd", NULL };

struct jtag_interface oscan1_ftdi_interface = {
	.name = "oscan1_ftdi",
	.supported = DEBUG_CAP_TMS_SEQ,
	.commands = oscan1_ftdi_command_handlers,
	.transports = oscan1_ftdi_transports,
	.swd = &oscan1_ftdi_swd,

	.init = oscan1_ftdi_initialize,
	.quit = oscan1_ftdi_quit,
	.speed = oscan1_ftdi_speed,
	.speed_div = oscan1_ftdi_speed_div,
	.khz = oscan1_ftdi_khz,
	.execute_queue = oscan1_ftdi_execute_queue,
};
